...
static int max30101_decoder_get_frame_count(const uint8_t *buffer, struct sensor_chan_spec chan_spec,
				 ...
				 uint32_t *fit, uint16_t max_count, void *data_out)
{
	...
	case SENSOR_CHAN_DIE_TEMP:
		if (edata->has_temp) {
			out->readings[0].temperature = (edata->reading.die_temp[0] << MAX30101_TEMP_FRAC_SHIFT) | (edata->reading.die_temp[1] & 0x0f);
	...
	out->readings[0].light = edata->reading.raw[data->map[led_chan][chan_spec.chan_idx]];
