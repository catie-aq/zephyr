...
//	LOG_WRN("max30101_submit_stream");
	for (size_t i = 0; i < cfg->count; i++) {
		if (cfg->triggers[i].trigger == SENSOR_TRIG_DATA_READY) {
			...
			if (cfg->triggers[i].opt == SENSOR_STREAM_DATA_INCLUDE) {
				LOG_ERR("MAX30101 OVERFLOW trigger does not support SENSOR_STREAM_DATA_INCLUDE");
				return;
			}
...
//	LOG_WRN("max30101_read_status_cb");
	...
	uint8_t rdy_event = 0, watermark_event = 0, overflow_event = 0;

	if (data_rdy != NULL) {
		rdy_event = data->status[0] & MAX30101_INT_PPG_MASK;
//		LOG_INF("Data ready: [%02x][%02x](%d) [%d]", data->status[0], MAX30101_INT_PPG_MASK, rdy_event, data->temp_available);
		operation_completed = (data_rdy->opt == SENSOR_STREAM_DATA_NOP) || (rdy_event == 0);
#if CONFIG_MAX30101_DIE_TEMPERATURE
		data->temp_available |= !!(data->status[1] & MAX30101_INT_TEMP_MASK);
//		LOG_INF("Data ready: [%02x][%02x](%d) [%d]", data->status[1], MAX30101_INT_TEMP_MASK, rdy_event, data->temp_available);
#endif /* CONFIG_MAX30101_DIE_TEMPERATURE */
	}

	if (watermark != NULL) {
		watermark_event = data->status[0] & MAX30101_INT_FULL_MASK;
		operation_completed |= (watermark->opt != SENSOR_STREAM_DATA_NOP) || (watermark_event == 0);
	}

	if (overflow != NULL) {
		overflow_event = data->status[0] & MAX30101_INT_ALC_OVF_MASK;
		operation_completed |= !!overflow_event;
	}
...
//		LOG_ERR("max30101_read_status_cb NOP");
	...
	if (!!rdy_event) {
		...
		if (!!data_channel) {
			...
			if (acquirable < 2) {
				LOG_ERR("Not enough sqes available: [%d/%d]", 2, acquirable);
				rtio_iodev_sqe_err(data->streaming_sqe, -ENOMEM);
				return;
			}
